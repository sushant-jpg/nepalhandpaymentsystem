...
const querySchema = z.object({
  ...
  status: z.enum(["PENDING", "SUCCESS", "FAILED", "DECLINED", "CANCELLED", "REFUNDED"]).optional(),
...
router.get("/:id", asyncHandler(async (req, res) => {
  ...
  res.json({ success: true, data: { transactionId: t.transactionId, customerName: t.customerId.displayName, merchantName: t.merchantId.businessName, amount: fromPaisa(t.amountPaisa), currency: t.currency, status: t.status, paymentMethod: t.paymentMethod, riskLevel: t.riskLevel, createdAt: t.createdAt, completedAt: t.completedAt } });
}));
...
router.get("/:id/receipt.pdf", authorize("CUSTOMER", "MERCHANT", "ADMIN", "AUDITOR"), asyncHandler(async (req, res) => {
  ...
  doc.moveDown().fillColor("#16835c").fontSize(17).text("Payment Successful");
  doc.moveDown(1.5).fillColor("#1e293b").fontSize(11);
  const rows = [["Merchant", t.merchantId.businessName], ["Amount", `NPR ${fromPaisa(t.amountPaisa).toLocaleString("en-NP", { minimumFractionDigits: 2 })}`], ["Transaction", t.transactionId], ["Date", new Intl.DateTimeFormat("en-NP", { dateStyle: "medium", timeStyle: "short", timeZone: "Asia/Kathmandu" }).format(t.completedAt)], ["Payment method", "Palm Wallet"], ["Status", t.status]];
