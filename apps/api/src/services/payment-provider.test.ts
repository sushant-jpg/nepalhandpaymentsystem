...
describe("demo wallet provider", () => {
  it("rejects an overdraft during authorization", async () => {
    ...
    await expect(new DemoWalletProvider().verifyPayment("customer", 5_001)).rejects.toMatchObject({ code: "INSUFFICIENT_BALANCE" });
  });
  ...
  it("debits and credits using guarded server-side updates", async () => {
    ...
    await new DemoWalletProvider().capturePayment("customer", "merchant", 10_000, session);
  ...
  it("fails the transfer if the conditional debit loses a race", async () => {
    ...
    await expect(new DemoWalletProvider().capturePayment("customer", "merchant", 10_000, session)).rejects.toMatchObject({ code: "INSUFFICIENT_BALANCE" });
  });
  ...
  it("reverses the direction for a refund", async () => {
    ...
    await new DemoWalletProvider().refundPayment("customer", "merchant", 2_500, session);
